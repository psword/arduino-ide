...
void setup() {
  ...
  int bootSequence = 1;        // Initialize boot priority
  delay(2500*bootSequence);    // Wait for "x" seconds after boot to reduce init
  // Initialize the LCD
  lcd.init(); 
  ...
  // First time delay for stability of the system
  if (!codeExecuted) {
    delay(30000);
    codeExecuted = true;
  }
