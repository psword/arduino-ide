...
#define BUFFER_SIZE 64 // Size of the buffer to store received data
  ...
  if (nextPing) {
    ...
    while (Wire.available()) { // Slave may send less than requested
      ...
      if (messageType == 0x02 && messageLength == 8) {
        ...
        // Testing this method...again
        int index = 0; // Index for the buffer
        // Read bytes from the I2C bus and append them to the buffer
       char receivedFloatValue[BUFFER_SIZE];    // Variable to store received float value
        while (Wire.available() && index < BUFFER_SIZE - 1) {
          char c = Wire.read(); // Read a byte from the I2C bus
          receivedFloatValue[index] = c; // Append the byte to the buffer
        ...
        // float *ptr = reinterpret_cast<float*>(byteArray); // Type punning
        // receivedFloatValue = *ptr; // Assign the reconstructed float value
        float receivedFloatTemp = atof(receivedFloatValue); // Convert char array to float
      ...
      if (messageType == 0x03 && messageLength == 8) {
       ...
       char receivedFloatValue[BUFFER_SIZE];    // Variable to store received float value
        ...
        // float *ptr = reinterpret_cast<float*>(byteArray); // Type punning
        // receivedFloatValue = *ptr; // Assign the reconstructed float value
        float receivedFloatTds = atof(receivedFloatValue); // Convert char array to float
